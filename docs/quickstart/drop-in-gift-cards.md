...
Integrating into Lightrail is an easy process that can be completed no more than a few days.

The solution is component based, using widgets which are created from simple HTML snippets.
When your customers receive a gift card, the can easily be redeemed and applied to the customer's account which can be used as a payment option during checkout. 
...
```
This gives your customer the information to choose whether or not to apply their account credit to their purchase, or set a specific amount of account credit to use. In our [sample webapp](https://github.com/Giftbit/stripe-integration-sample-webapp/blob/master/shared/views/checkout.html), the customer simply selects a checkbox to apply their account credit.
...
This should be familiar to you, because you'll be calling methods in our client libs that are similar to the calls you make to Stripe. 

This script will need to do the following: 

- Let the customer choose whether to apply their account credit
...
They are based on a unique customer identifier from your ecommerce system: the `shopperId`. This is what links the customer from your system to their account in Lightrail. 
