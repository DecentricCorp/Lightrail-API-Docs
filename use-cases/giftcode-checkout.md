...
This document is quick guide to implementing gift code redemeption at the order checkout in an e-commerce solution by directly calling the Lightrail API. If you are looking for client libraries specific to a language or technology, check out the Integration projects in the Lightrail API documentation.

## Use-Case

Redemption at the checkout is perhaps the most common use-case for a gift code. After creating an order, a customer who have previously received a gift code can redeem it at the checkout so that the value of the gift code can pay for part, or all of the order balance.

The common flow for this is to provice the customer with a space to enter the gift code at the checkout time, determine the value of the gift code, and adjust the order balance accordingly. If the gift code value is sufficient for covering the entire order balance, the balance is charged on the gift code and the checkout will be complete. If the gift code value is not sufficient to cover the whole order, the gift code is charged with whatever it can pay, i.e. its available value, and the remaning balance is paid using a third-party payment method such as a credit card.
...
- If the gift code value has a different currency that the value of the order, the user should receive an error message. Note that Lightrail does not support currency exchange.
- If the gift code value is zero or it is inactive the customer should be notified of this as an error.
- Since some third-party payment methods have a restriction on the minimum transaction value, there is an edge case where the gift code value is too small to pay for the entire order, but also the order balance is so small that the remaining balance after taking out the gift code's will be too small for the third-party transaction. In these rare cases, the customer should be notified that gift code value is too small for that transaction.
...
2. Use the [Balance API endpoint](#balance-endpoint) to determine the available value of the code (`giftCodeValue`). Prompt the user with an error if:
   - The gift code is not `ACTIVE`, 
...
4. **If `giftCodeValue < orderBalance`**, meaning the gift code value is not enough for paying for the entire order, the gift code will pay for part of the balance (`giftCodeShare`) and the remainder (i.e. `orderBalance-giftCodeShare`) will be paid via another payment method. 
   ...
   if (stripeShare > 0 && stripeShare < STRIPE_MINIMUM_TRANSACTION_VALUE) {
     ...
     giftCodeShare = giftCodeShare - differential;
     if (newGiftCodeShare < 0)
       throw new InsufficientValueException
         ("This gift code value is too small for this transaction.");
    }
   ```
...
After posting a `pending` transaction, you can use this endpoint to either capture or void it. Note that in order to call this endpoint you will need the `cardId` corresponding to the gift code and the `transactionId` for the original pending transaction which are provided in the response object from that call.
